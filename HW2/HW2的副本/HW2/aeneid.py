...
def compute_links(result, limit=None, offset=None):
    ...
    pre_offset = int(offset) - int(limit)
    ...
    pre_args["limit"] = limit
    pre_args["offset"] = pre_offset

    next_params = urlencode(next_args)
    pre_params = urlencode(pre_args)
    next_self = {"rel": "next", "href": base + "?" + next_params}
    pre_self = {"rel": "previous", "href": base + "?" + pre_params}
    result["links"].append(next_self)
    result["links"].append(pre_self)
...
def handle_resource(dbname, resource_name, primary_key):
    ...
    try:
        ...
        if request.method == "GET":
            ...
            resp = Response("I am a teapot that will not have much more functions", status=422, mimetype='text/plain')
            ...
            if result:
                ...
                for r in result:
                    for k, v in r.items():
                        for f in field_list:
                            if k == f.split('.')[1]:
                                if not f.split('.')[0] in record:
                                    record[f.split('.')[0]] = {}
                                record[f.split('.')[0]][f.split('.')[1]] = v
                    record_copy = copy.deepcopy(record)
                    final_result.append(record_copy)
