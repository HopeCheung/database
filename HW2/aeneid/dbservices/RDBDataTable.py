...
class RDBDataTable(BaseDataTable):
    ...
    def _get_extras(self, limit, offset= None, order_by=None):
        result = ' '
        if order_by:
    ...
    def join_by_template(self, template, child, cons, field_list=None, limit=None,
                         offset=None, order_by=None, commit=True):
        result = None
        ...
        try:
            ...
            ext = self._get_extras(limit, offset, order_by)
            referenced_table_name = self._table_name.split(".")[1]
            q = "select {} from " + referenced_table_name + " join " + child + " on "
            ...
            if field_list is None:
                ...
                for i in range(len(field_list)):
                    if "." not in field_list[i]:
                        field_list[i] = referenced_table_name + "." + field_list[i]
                f_select = field_list

            # for child in children:
            #     q = q + " join " + child + " on " + referenced_table_name + "." \
            #         + cons[referenced_table_name]
            #     q = q + "=" + child + "." + cons[child]
            # q = q + " " + w_clause[0]
            for c in cons:
                q = q + referenced_table_name + "." + c + "=" + child + "." + c + " and "
            q = q.strip("and ") + " " + w_clause[0]
    ...
    def update_by_template(self, template, new_values):
        ...
        q = "update " + self._table_name + " set " + ", ".join(terms) + " " + wc[0]
